...
export default function QuotationList() {
  ...
  const duplicate = async (id) => {
    try {
      ...
      navigate(`/bao-gia-minh-triet/${newId}/chinh-sua`);
  ...
  const createNew = () => {
    window.localStorage.removeItem(`quotation:draft:${user?.id || 'anonymous'}:new`);
    navigate('/bao-gia-minh-triet/tao-moi');
  };

  return <><Helmet><title>Danh sách báo giá</title></Helmet><div className={styles.page}>
    <header className={styles.pageHeader}><div><span>QUẢN LÝ BÁO GIÁ</span><h1>DANH SÁCH BÁO GIÁ</h1></div><button type="button" className={styles.primaryLink} onClick={createNew}><FaPlus/> Tạo báo giá</button></header>
    ...
    <div className={styles.tableWrap}><table className={styles.listTable}><thead><tr><th>Số báo giá</th><th>Ngày</th><th>Khách hàng</th><th>Liên hệ</th><th>Tổng tiền</th><th>Nhân viên</th><th>Trạng thái</th><th>Ngày tạo</th><th>Thao tác</th></tr></thead><tbody>
      {loading ? <tr><td colSpan="9">Đang tải...</td></tr> : rows.length === 0 ? <tr><td colSpan="9">Chưa có báo giá.</td></tr> : rows.map((row) => <tr key={row.id}><td><Link to={`/bao-gia-minh-triet/${row.id}/chinh-sua`}>{row.quotation_no}</Link></td><td>{row.quotation_date}</td><td>{row.customer_name}</td><td>{row.contact_name || row.phone || '—'}</td><td className={styles.money}>{formatCurrency(row.total)}</td><td>{row.prepared_by_name || '—'}</td><td><span className={`${styles.status} ${styles[row.status]}`}>{QUOTATION_STATUSES[row.status]}</span></td><td>{new Date(row.created_at).toLocaleString('vi-VN')}</td><td><div className={styles.rowActions}><button title="Chỉnh sửa" onClick={() => navigate(`/bao-gia-minh-triet/${row.id}/chinh-sua`)}><FaPen/></button><button title="Tạo bản sao" onClick={() => duplicate(row.id)}><FaCopy/></button><button title="Xuất Excel" onClick={() => excel(row.id)}><FaFileExcel/></button><button title="Xuất PDF" onClick={() => navigate(`/bao-gia-minh-triet/${row.id}/chinh-sua?pdf=1`)}><FaFilePdf/></button><button title="In" onClick={() => navigate(`/bao-gia-minh-triet/${row.id}/chinh-sua?print=1`)}><FaPrint/></button><button title={row.status === 'draft' ? 'Xóa' : 'Hủy'} className={styles.danger} onClick={() => remove(row)}>{row.status === 'draft' ? <FaTrash/> : <FaBan/>}</button></div></td></tr>)}
    </tbody></table></div></section>
    <ConfirmModal
      open={!!confirmRow}
      title={confirmRow?.status === 'draft' ? 'Xóa bản nháp' : 'Hủy báo giá'}
      message={confirmRow?.status === 'draft' ? 'Xóa bản nháp này?' : 'Hủy báo giá này?'}
      confirmLabel={confirmRow?.status === 'draft' ? 'Xóa' : 'Hủy báo giá'}
      danger
